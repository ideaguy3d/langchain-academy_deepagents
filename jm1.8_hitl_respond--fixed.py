"""HITL email demo with a terminal-backed multiple-choice ask_user tool."""
...
from jmodels import model


@tool
def ask_user(question: str, choices: list[str]) -> str:
    """Ask the user to choose one of the supplied options in the terminal."""
    answer = questionary.select(question, choices=choices).ask()
    if answer is None:
        raise RuntimeError("No choice selected; the clarification was cancelled.")
    return answer
...
- When confirming an email was sent, quote the subject and body from the tool result.
"""
...
agent = create_deep_agent(
    ...
    # ask_user executes normally so it can render its choices in the terminal.
    # send_email still pauses for an explicit human approval decision.
    interrupt_on={
...
config = {"configurable": {"thread_id": "jm1-8-hitl-respond-fixed-demo"}}
